  ...
  class __CLASSNAME__ {
      ...
      if ($this->listening != false) {
            ...
            foreach ($this->authorizedUsers as $nsaccount) {
              if (strtolower($message[0])
                ...
                $module->$callback($this->queue[$this->listening][0],
                  $this->listening, $this->queue[$this->listening][3], true);
              }
            }
          }
        }
        elseif ($numeric == 318) {
          ...
          if (strtolower($message[0])
            ...
            $callback = $this->queue[$this->listening][2];
            $module->$callback($this->queue[$this->listening][0],
              $this->listening, $this->queue[$this->listening][3], false);
          }
    ...
    public function isInstantiated() {
      ...
      EventHandling::registerForEvent("numericEvent", $this,
        "receiveNumericEvent", 330); // Numeric for "X :is logged in as"
      EventHandling::registerForEvent("numericEvent", $this,
        "receiveNumericEvent", 318); // End of WHOIS
      return true;
    }
  }
?>
