<?php
	class @@CLASSNAME@@ {
		...
		public function receiveChannelMessage($name, $data) {
			...
			if (preg_match("/^".$connection->getNickname().": join (.*)/", $message, $matches)) {
			...
			if (preg_match("/^".$connection->getNickname().": part (.*)/", $message, $matches)) {
				$module = ModuleManagement::getModuleByName("UserIdentification");
				if (is_object($module)) {
					if (stristr($matches[1], ",")) {
						$matches[1] = explode(",", $matches[1]);
			...
			if (preg_match("/^".$connection->getNickname().": autojoin (.*)/", $message, $matches)) {
				$module = ModuleManagement::getModuleByName("UserIdentification");
				if (is_object($module)) {
					if (stristr($matches[1], ",")) {
						$matches[1] = explode(",", $matches[1]);
			...
			if (preg_match("/^".$connection->getNickname().": autopart (.*)/", $message, $matches)) {
				$module = ModuleManagement::getModuleByName("UserIdentification");
				if (is_object($module)) {
					if (stristr($matches[1], ",")) {
						$matches[1] = explode(",", $matches[1]);
