<?php
  class @@CLASSNAME@@ {
              ...
              if ($id > 0) {
                ...
                if (in_array(substr($val, 0, 1),
                  ...
                  foreach ($access as $mode) {
                    $this->queue[$this->listening][4][] = array($mode,
                      $channel);
                  }
                }
              }
        ...
        elseif ($numeric == 318) {
          ...
          if (strtolower($message[0])
            ...
            $module->$callback($this->queue[$this->listening][0],
              $this->listening, $this->queue[$this->listening][3],
              $this->queue[$this->listening][4]);
          }
    ...
    public function isInstantiated() {
      EventHandling::registerForEvent("numericEvent", $this, "receiveEvent",
        319); // Numeric for "X :@#torrents @#channel +#terminal"
      EventHandling::registerForEvent("numericEvent", $this, "receiveEvent",
        318); // End of WHOIS
      return true;
    }
